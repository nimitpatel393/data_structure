 ...
 * @param row_ptr -> to the score row.
 */
void fill_score_row(ifstream& input, int score_count, RowPtr row_ptr);
...
int main()
{
    ...
    string course;
    NameArray names = nullptr;  // -> dynamic array of student names
    RowArray  rows  = nullptr;  // -> dynamic array of pointers to the
                         ...
                         NameArray names, RowArray rows)
{
    ...
    RowPtr  row_ptr  = rows;   // -> scores row
    ...
    while (student_count > 0)
    {
        ...
        *name_ptr = first_name + " " + last_name;
        ...
        int *scores = new int[score_count + 1];
        *row_ptr = scores;
        fill_score_row(input, score_count, row_ptr);

        name_ptr++;       // -> next name of the name array
        row_ptr++;        // -> next scores row
...
void print_student_scores(const string course, int student_count,
                          NameArray names, RowArray rows)
{
    NamePtr name_ptr = names;  // -> first name in the names array
    RowPtr  row_ptr  = rows;   // -> first scores row
    ...
    while (student_count > 0)
    {
        ...
        name_ptr++;       // -> next name in the names array
        row_ptr++;        // -> next scores row
        student_count--;  // decrement the copy of the student count
    }
}
